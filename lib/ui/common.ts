// Note: use DOM HTMLElement from Obsidian, not node-html-parser's HTMLElement

export function flomoDate(daysAgo = 0): string[] {
  const date = new Date();
...
export function createExpOpt(contentEl: HTMLElement, label: string): HTMLInputElement {
  ...
  const optBox: HTMLInputElement = expOptionLabel.createEl('input', {
    type: 'checkbox',
    cls: 'ckbox'
  });
